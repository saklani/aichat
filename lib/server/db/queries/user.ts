...
export async function createUser(data: Pick<schema.User, "googleId" | "email" | "passwordHash">) {
    return execute('insert user', () => db.insert(schema.users).values({ ...data }).returning().then(res => res.at(0)))
}
...
export async function getUserByGoogleId({ googleId }: Pick<schema.User, "googleId">) {
    return execute(`get user ${googleId}`, () => db.query.users.findFirst({ where: eq(schema.users.googleId, googleId!), columns: { id: true, email: true } }))
}
